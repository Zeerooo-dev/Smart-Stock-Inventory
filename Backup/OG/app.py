...
from main_window_ui import Ui_MainWindow, BASE_STYLE, COLOR
...
class SmartStockApp(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, is_sandbox=False):
        ...
        # Environment-isolated barcode cache directories
        if is_sandbox:
            self.barcode_dir = "cache_test_barcodes"
        else:
            self.barcode_dir = "cache_barcodes"
    ...
    def _set_nav_active(self, active_btn):
        for btn in (self.btn_nav_dashboard, self.btn_nav_reports, self.btn_nav_settings):
    ...
    def _write_ledger(self, cursor, item_id, delta_qty, price_snapshot, change_type):
        cursor.execute(
            ...
            "(ItemID, DeltaQuantity, PriceSnapshot, ChangeType) VALUES (?,?,?,?)",
            (item_id, delta_qty, price_snapshot, change_type),
        )
        ...
        if HAS_BARCODE:
            try:
                Code128(sku, writer=ImageWriter()).save(
                    os.path.join(self.barcode_dir, sku)
                )
                return os.path.join(self.barcode_dir, f"{sku}.png")
            except Exception:
                pass
        return None
    ...
    def _show_barcode_preview(self, image_path, sku=""):
        if image_path and os.path.isfile(image_path):
            ...
            # Delete the physical file immediately after loading into memory
            try:
                os.remove(image_path)
            except Exception as e:
                print(f"Non-blocking cleanup warning: {e}")
            if not pix.isNull():
    ...
    def _populate_inventory_table(self, rows):
        ...
        for idx, row in enumerate(rows):
            ...
            for col, text in enumerate(display):
                ...
                if col == 1:
                    barcode_path = os.path.join(self.barcode_dir, f"{sku}.png") if sku else ""
    ...
    def add_item(self):
        ...
        try:
            with sqlite3.connect(self.db_name) as conn:
                ...
                item_id = c.lastrowid
                self._write_ledger(c, item_id, qty, price, "CREATE")
        ...
        try:
            with sqlite3.connect(self.db_name) as conn:
                ...
                if delta_qty != 0 or new_price != old_price:
                    self._write_ledger(
                        c, item_id, delta_qty, new_price, "MANUAL_EDIT"
                    )
        except sqlite3.Error as e:
            QtWidgets.QMessageBox.critical(self, "Database Error", str(e))
            return
    ...
    def _adjust_stock(self, direction):
        ...
        try:
            with sqlite3.connect(self.db_name) as conn:
                ...
                self._write_ledger(c, item_id, delta, price, change_type)
        except sqlite3.Error as e:
            QtWidgets.QMessageBox.critical(self, "Database Error", str(e))
            return
        self.load_table_data()
        ...
        try:
            with sqlite3.connect(self.db_name) as conn:
                ...
                for row_num, row in enumerate(rows, start=2):
                    ...
                    item_id = c.lastrowid
                    self._write_ledger(c, item_id, qty, price, "CSV_IMPORT")
                    count += 1
    ...
    def closeEvent(self, event):
        """Wipe the sandbox barcode cache directory on exit when in test mode."""
        if self.db_name == "smartstock_sandbox.db":
            try:
                if os.path.isdir(self.barcode_dir):
                    shutil.rmtree(self.barcode_dir)
            except Exception as e:
                print(f"Non-blocking sandbox cleanup warning: {e}")
        event.accept()
