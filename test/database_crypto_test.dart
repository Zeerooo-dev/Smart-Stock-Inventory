...
void main() {
  ...
  Future<Uint8List> makeLegacy() async {
    ...
    final key = SecretKey((await Sha256().hash(utf8.encode('test-only-passphrase'))).bytes);
    final box = await cipher.encrypt(original, secretKey: key);
    final payload = Uint8List.fromList([...utf8.encode('SSG1'), ...box.nonce, ...box.mac.bytes, ...box.cipherText]);
  ...
  test('new key round-trip survives a fresh crypto instance', () async {
    ...
    expect(String.fromCharCodes((await factory.readDatabaseBytes('$path.enc')).take(4)), 'SSG2');
  ...
  test('missing key preserves ciphertext and creates no replacement key', () async {
    final crypto = DatabaseCrypto(factory, path, keyStore: keys);
    await crypto.decryptIfNeeded();
    await crypto.encryptAndDeletePlaintext();
    final encrypted = await factory.readDatabaseBytes('$path.enc');
    keys.values.clear();
    await expectLater(DatabaseCrypto(factory, path, keyStore: keys).decryptIfNeeded(), throwsStateError);
    expect(await factory.readDatabaseBytes('$path.enc'), encrypted);
    expect(await factory.databaseExists(path), isFalse);
    expect(keys.values, isEmpty);
  });

  test('legacy unlock requires a passphrase and wrong passphrase preserves data', () async {
    final encrypted = await makeLegacy();
    final crypto = DatabaseCrypto(factory, path, keyStore: keys);
    await expectLater(crypto.decryptIfNeeded(), throwsA(isA<LegacyDatabaseKeyRequired>()));
    await expectLater(crypto.decryptIfNeeded(legacyPassphrase: 'incorrect-test-key'), throwsA(isA<SecretBoxAuthenticationError>()));
    expect(await factory.readDatabaseBytes('$path.enc'), encrypted);
    expect(await factory.databaseExists(path), isFalse);
    expect(keys.values, isEmpty);
  });

  test('legacy migration keeps recovery copy and next encryption uses random key', () async {
    final encrypted = await makeLegacy();
    final crypto = DatabaseCrypto(factory, path, keyStore: keys);
    await crypto.decryptIfNeeded(legacyPassphrase: 'test-only-passphrase');
    expect(await factory.readDatabaseBytes(path), original);
    expect(await factory.readDatabaseBytes('$path.enc.legacy-backup'), encrypted);
    await crypto.encryptAndDeletePlaintext();
    await DatabaseCrypto(factory, path, keyStore: keys).decryptIfNeeded();
    expect(await factory.readDatabaseBytes(path), original);
  });

  test('secure storage failure during legacy migration leaves original intact', () async {
    final encrypted = await makeLegacy();
    keys.failWrites = true;
    await expectLater(DatabaseCrypto(factory, path, keyStore: keys).decryptIfNeeded(legacyPassphrase: 'test-only-passphrase'), throwsStateError);
    expect(await factory.readDatabaseBytes('$path.enc'), encrypted);
    expect(await factory.databaseExists(path), isFalse);
  });

  test('both plaintext and ciphertext are preserved when recovery is ambiguous', () async {
    final encrypted = await makeLegacy();
  ...
  test('failed initialization cannot encrypt or delete a plaintext database', () async {
    final crypto = DatabaseCrypto(factory, path, keyStore: keys);
    keys.failWrites = true;
