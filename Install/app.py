...
# ─────────────────────────────────────────────────────────────────────
#  FILE-LEVEL DATABASE ENCRYPTION (cryptography / Fernet) + APPDATA PATH
#
#  Plain SQLite is used while the app is running (fast, no compiler
#  needed). On exit, the whole .db file is encrypted to .db.enc and the
#  plaintext is deleted. On launch, .db.enc is decrypted back to a
#  working .db. Result: nothing readable ever sits on disk between runs.
# ─────────────────────────────────────────────────────────────────────
from cryptography.fernet import Fernet
import base64
import hashlib

# Change this to your own secret passphrase. Anyone with this string
# (and the .db.enc file) can decrypt your data, so keep it private —
# e.g. load it from an environment variable instead of hardcoding it.
...
def _resource_path(relative_path):
    """Resolves a bundled resource path whether running as a plain script
    or as a PyInstaller-built .exe (where files are unpacked to a temp dir)."""
