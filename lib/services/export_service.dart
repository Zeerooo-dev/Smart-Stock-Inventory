...
class ExportService {
  ...
  Future<Uri?> exportAuditCsv(AuditFilter filter) async {
    ...
    final rows = <List<Object?>>[
      ['Timestamp (UTC)', 'Item Name', 'SKU', 'Change Type', 'Reference', 'Delta Quantity', 'Price Snapshot', 'Running Balance', 'Notes'],
      ...entries.map((e) => [
            e.timestamp,
            _safeCsvCell(e.itemName),
            e.sku,
            e.changeType,
            _safeCsvCell(e.sourceReference),
            e.deltaQuantity,
            e.priceSnapshot,
            e.runningBalance,
            _safeCsvCell(e.notes),
          ]),
    ];
    final data = utf8.encode(Csv().encode(rows));
    return FilePicker.saveFile(
  ...
  Future<Uri?> exportItemHistoryCsv(int itemId, String itemName) async {
    ...
    final rows = <List<Object?>>[
      ['Timestamp (UTC)', 'Change Type', 'Reference', 'Delta Quantity', 'Price Snapshot', 'Running Balance', 'Notes'],
      ...entries.map((e) => [
            e.timestamp,
            e.changeType,
            _safeCsvCell(e.sourceReference),
