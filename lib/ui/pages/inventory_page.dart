...
class _InventoryPageViewState extends State<InventoryPageView> {
  ...
  Future<void> _adjustItem(InventoryItem item, {required bool restock}) async {
    widget.controller.selectItem(item);
    final amount = await showStockAmountDialog(context, restock: restock);
    if (amount == null) return;
    try {
      await widget.controller.adjustStock(item, amount, restock: restock);
    } catch (e) {
      _error('$e');
    }
  }
        ...
        child: Column(
          ...
          children: [
            ...
            _SheetAction(
              ...
              onTap: () {
                Navigator.pop(sheetContext);
                _adjustItem(item, restock: false);
              },
            ),
            _SheetAction(
              icon: Icons.history,
